...
class ProdHospitalRepository extends HospitalRepository {
  async getPatientDetail(patient_id: string): Promise<Patient> {
    ...
    const result: Patient = await fetch(
      `${O3_BASE_URL}/patient/${patient_id}?v=full`,
      requestOptions
    )
      .then((response) => {
        if (response.ok) {
  ...
  async getUser(user_id: string): Promise<User> {
    ...
    const result: User = await fetch(
      `${O3_BASE_URL}/user/${user_id}?v=full`,
      requestOptions
    )
      .then((response) => {
        if (response.ok) {
  ...
  async getDoctor(person_id: string): Promise<User> {
    ...
    const result: User = await fetch(
      `${O3_BASE_URL}/person/${person_id}?v=custom:(uuid,display)`,
      requestOptions
    )
      .then((response) => {
        if (response.ok) {
  ...
  async getVisits(patient_id: string): Promise<Array<Visit>> {
    ...
    await fetch(
      `${O3_BASE_URL}/encounter?patient=${patient_id}&v=custom:(visit:(visitType:(display),startDatetime,stopDatetime,encounters:(display,encounterType:(display))),encounterDatetime,obs:(concept:(display),obsDatetime,value))`,
      requestOptions
    )
      .then((response) => {
        if (response.ok) {
      ...
      .then((result) => {
        ...
        data.forEach(({ visit }) => {
          visits.push({
            ...
            encounters: (visit.encounters as Array<any>).map((encounter) => {
              return {
